...
TextDetector::TextDetector()
{
	...
	string model_path = "inference/ch_PP-OCRv4_det_server_infer/model.onnx";
...
Mat TextDetector::preprocess(Mat srcimg ,float &ratio_h, float &ratio_w)
{
  ...
  std::cout << "resize_h:" << resize_h << " resize_w:" << resize_w << std::endl;
...
void TextDetector::normalize_(Mat *im)
{
	//    img.convertTo(img, CV_32F);
    std::vector<float> mean = {0.485f, 0.456f, 0.406f};
    std::vector<float> scale = {4.36681f,4.46429f, 4.44444f};
    bool is_scale = 1;
    double e = 1.0;
    if (is_scale) {
      e /= 255.0;
...
vector< vector<Point2f> > TextDetector::detect(Mat& srcimg)
{
    ...
    float ratio_h;
    float ratio_w;
	Mat dstimg = this->preprocess(srcimg,ratio_h,ratio_w);
	//this->normalize_(dstimg);
	this->normalize_(&dstimg);
    std::cout << "dstimg.rows: " << dstimg.rows << " dstimg.cols:" << dstimg.cols <<std::endl;
    std::vector<float> input(1 * 3 * dstimg.rows * dstimg.cols, 0.0f);
    Permute(&dstimg, input.data());
